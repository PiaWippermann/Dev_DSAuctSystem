...
def auction_listener_leader():
    ...
    if s_address == leader:
        ...
        b_listener.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        # The leader is the discovery listener
        b_listener.bind((s_address, AUCTION_PORT))
        print(f'Servers address as broadcast listener: {s_address}')
...
def auction_status_listener():
    if s_address != leader:
        ...
        b_listener.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        # The leader is the discovery listener
        b_listener.bind((s_address, BROADCAST_PORT_AUCTION))
        print(f'Server {s_address} is bound to auction status listener')
...
def message_to_server(message):
    ...
    sender_socket.sendto(server_list.encode(),
                         ('<broadcast>', MESSAGE_TO_SERVER_PORT))
    print('Other servers informed')
    sender_socket.close()


# listener for incoming messages from servers to get the updated server list
# TODO: check ob man das so umändern kann, dass server liste und der auctions status an alle Server weitergegeben werden kann
